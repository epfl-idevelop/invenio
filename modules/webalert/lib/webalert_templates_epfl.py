...
from invenio import webalert_templates

class Template(webalert_templates.Template):
...
%(table)s
        ...
        msg = _("You can set a new alert from %(x_url_open)syour searches%(x_url_close)s.")
        msg %= {'x_url_open': '<a href="display?ln=' + ln + '">',
                'x_url_close': '</a>',
                }
        ...
        if len(alerts):
          ...
          <table class="table">
            <tr>
              ...
              <th>%(result_basket)s</th>
              <th>%(date_run)s</th>
              <th>%(date_created)s</th>
              <th>%(query)s</th>
              <th>%(action)s</th>
          </tr>""" % {'name' : _("Name"),
...
%(table)s
        ...
        msg = ''
        if len(queries) == 0:
            msg = _("You have not executed any search yet. Please go to the %(x_url_open)ssearch interface%(x_url_close)s first.") % \
                {'x_url_open': '<a href="' + CFG_SITE_URL + '/?ln=' + ln +'">',
                 'x_url_close': '</a>'}
            return out % {'create_label': _("New personal collection"), 
                          'ln': ln, 
                          'label': _("Performed searches"), 
                          'msg': msg, 
                          'table': '' }
        ...
        for query in queries :
            ...
            <tr>
              ...
              <td style="white-space: nowrap">
                <a href="%(siteurl)s/search?%(args)s">%(execute_query)s</a><br />
                <a href="%(siteurl)s/youralerts/input?ln=%(ln)s&amp;idq=%(id)d">%(set_alert)s</a>
              </td>""" % {
                     'index' : i,
            ...
            if permanent == "n":
                table += """
              <td style="white-space: nowrap">%s</td>""" % query['lastrun']
            table += """
            </tr>"""
        table += """
          </table>"""
